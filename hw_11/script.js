...
let colls = document.querySelectorAll('*');

function taskOne(myNodeList) {
  for (let  i = 0; i < myNodeList.length; i++){
      if (myNodeList[i].innerHTML.includes('2021')){
      let node = myNodeList[i].innerHTML.replace('2021','2020');
...
`
const nav = document.querySelector('table').lastElementChild.lastElementChild;
...
function taskFive() {
  const row = document.querySelector('table').firstElementChild.firstElementChild.nextElementSibling;
  row.remove();
}
...
function taskFive(argument) {
  let table = document.querySelector('table');
  table.rows[argument].remove();
}
